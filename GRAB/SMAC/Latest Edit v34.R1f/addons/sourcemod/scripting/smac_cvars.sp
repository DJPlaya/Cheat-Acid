...
public OnPluginStart()
{
	...
	AddCvar(Order_First, "open",							Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "prepareuranus",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ZhykLua",						Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "zapuskscript",					Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ru_show_steam",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ru_steam_set_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ru_steam_set_random_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "show_steam",					Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "steam_set_id",					Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "steam_set_random_id",					Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "servishack_show_steam",		Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "servishack_steam_set_id",		Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "servishack_steam_set_random_id",		Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "starts",						Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "cs_show_steam",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "cs_steam_set_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "cs_steam_set_random_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ssshow_steam",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "sssteam_set_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "sssteam_set_random_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "dnashow_steam",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "dnasteam_set_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "dnasteam_set_random_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ServisHack_show_steam",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ServisHack_steam_set_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "ServisHack_steam_set_random_id",				Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "openhack",						Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "aaa123_show_steam",	Comp_NonExist, Action_Ban);
	AddCvar(Order_First, "aaa123_steam_set_id",	Comp_NonExist, Action_Ban);
	...
	{
		AddCvar(Order_Last, "voice_inputfromfile",			Comp_Equal, Action_Mute, "0.0");
	}
...
public OnCvarHldjChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if (GetConVarBool(g_hCvarHldj) == true)
	{
		AddCvar(Order_Last, "voice_inputfromfile", Comp_Equal, Action_Mute, "0.0");
		SMAC_PrintAdminNotice("HLDJ/HLSS protecting enabled.");
	}
	else
	{
		RemCvar("voice_inputfromfile");
		SMAC_PrintAdminNotice("HLDJ/HLSS protecting disabled.");
	}
}
	...
	{	
		...
		switch (CAction)
		{
			...
			case Action_Ban:
			{
				...
				SMAC_Ban(client, "ConVar violation, Hack Tool Detected");
			}
		}
	}
